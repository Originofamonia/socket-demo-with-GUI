...
class Server(threading.Thread):
    ...
    def __init__(self, host, port):
        ...
        self.root.title("Primary server status")  # set server gui title
    ...
    def run(self):
        ...
        print(f"Primary listening on {self.port}")
        ...
        while True:
            ...
            # for conn in self.connections:
                if conn.my_socket.fileno() != -1 and 'backup_server' == conn.username:  # send whole lexicon to backup
                    ...
                    server_command.payload = ' '.join(conn.correct_words)
                    packed_data = pickle.dumps(server_command)  # Serialize the class to a binary array
                    conn.my_socket.sendall(struct.pack("i", len(packed_data)))
                    conn.my_socket.sendall(packed_data)
...
def main():
    ...
    port1 = 7789  # edit the port number

    server1 = Server(host, port1)  # instantiate server object
    server1.start()  # start server thread
    server1.root.mainloop()  # tk mainloop
